...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'invoices' AND column_name = 'invoice_number') THEN
        ...
        -- Generate invoice numbers for existing records using a different approach
        WITH numbered_invoices AS (
            SELECT id, 'INV-' || EXTRACT(YEAR FROM created_at) || '-' || LPAD((ROW_NUMBER() OVER (ORDER BY created_at))::TEXT, 4, '0') as new_number
            FROM public.invoices
            WHERE invoice_number IS NULL
        )
        UPDATE public.invoices 
        SET invoice_number = numbered_invoices.new_number
